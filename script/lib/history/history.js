define(['lib/history/history'], function()
{
	// only use this in IE
//	if (Browser.name !== "ie")
//	{
	...
	(function( window ){
			...
			"redirect": function( type, basepath ){
				...
				if( window.top == window.self ){
					...
					if( isSupportHistoryAPI ){
						...
						windowLocation.replace(basepath + '#' + path.replace(new RegExp("^" + basepath, "i"), type) + search + windowLocation.hash);
					}
				}
			},
			/**
		...
		function parseURL( href, isWindowLocation, isNotAPI ){
			...
			if( href && !isWindowLocation ){
				...
				href = /^(?:[\w0-9]+\:)?\/\//.test(href) ? href.indexOf("/") === 0 ? _protocol + href : href : _protocol + "//" + current._host + (
		...
		function ie6DriverStart(){
			...
			var ua = navigator.userAgent.toLowerCase(), UA = ua.match(/(opera|ie|firefox|chrome|version)[\s\/:]([\w\d\.]+)?.*?(safari|version[\s\/:]([\w\d\.]+)|$)/) || [null, 'unknown', 0], b_mode = UA[1] == 'ie' && document.documentMode, b_name = (UA[1] == 'version') ? UA[3] : UA[1], b_version = b_mode || parseFloat((UA[1] == 'opera' && UA[4]) ? UA[4] : UA[2]);
