...
export class ValidationRule
{
	...
	constructor( elementTypes:Array<string>,
	             fn:(el:HTMLInputElement[],
	                 args:Array<string>,
	                 fn:(result:boolean,
	                 errorMsg?:string) => void, data?:any ) => void,
	             public message = '' )
	{
		this.fn = fn;
	...
	validate( el:HTMLElement[], args:Array<string>, fn:(result:boolean, errorMsg?:string) => void )
	{
...
export class FormValidator
{
	public static rules:{[id:string]:ValidationRule} =
	{
		...
		*/
		 ...
		 *      </div>
		 */
		'range': new ValidationRule(['input'], ( el, args, fn ) =>
		{
		 ...
		 */
		'email': new ValidationRule(['input'], ( el, args, fn ) =>
		{
		 ...
		 *      </div>
		 */
		'equal-to': new ValidationRule(['input'], ( el, args, fn ) =>
		{
			var values = [];
		 ...
		 *      </div>
		 */
		'minimum-length': new ValidationRule(['input'], ( el, args, fn ) =>
		{
			var value = $( el ).val() || ''
	...
	public static addValidationRule( id:string, rule:ValidationRule )
	{
	 ...
	 */
	static validateForm( el:HTMLFormElement, applyStyles:boolean = true, fn:(success:boolean) => void = () => {}):any
	{
		var $elements = $(el).find('[data-validations]');
		var validationSuccess = true;

		$elements.each((index, el:HTMLElement) =>
		{
			FormValidator.validate(el, (success, error) =>
			{
	 ...
	 *              error messages
	 */
	public static validate( el:HTMLElement, fn:( succes:boolean, error:any ) => void = () => { })
	{
		var errorMessages = [];
		var validationSuccess = true;
		...
		for (var i = 0; i < fieldRules.length; i++)
		{
			...
			if (typeof FormValidator.rules[ruleId] != 'undefined')
			{
				...
				FormValidator.rules[ruleId].validate(input, args, (success:boolean, message?:string) =>
				{
					if (validationSuccess == true)
					{
						validationSuccess = success;
